...
if ($Step1 -eq "Non-compliant" -and $OSPatch -eq "Compliant" -and $SecureBoot -eq "Compliant") {
    $Compliant = $false
    if ($Remediate) {
        Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\SecureBoot' -Name 'AvailableUpdates' -Type DWord -Value 0x40 -Force | Out-Null
