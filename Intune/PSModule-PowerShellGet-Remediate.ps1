...
$ModulesList = get-module -Name PowerShellGet -ListAvailable
if ($ModulesList.Count -gt 1){
    $NewestModule = $ModulesList | Where-Object {$_.Version -eq ($ModulesList | Select-Object -Property Version | Sort-Object | Select-Object -First 1).version}
    Import-Module -Name $ModuleName -RequiredVersion $NewestModule.Version -Force
    ForEach ($ModuleItem in $ModulesList | Where-Object {$_.Version -ne $NewestModule.Version})
        {
        Write-Output "Removing $($ModuleItem.name) $($ModuleItem.Version)"
        $RemovePath = $ModuleItem.Path | Split-Path
        Remove-Item -Path $RemovePath -Recurse -force
        }
    }
else
    {
    $NewestModule = $ModulesList
    }
...
if ($Status.Version -lt $RequiredVersion)
    ...
    Install-Module -Name $ModuleName -Force -AllowClobber -SkipPublisherCheck
