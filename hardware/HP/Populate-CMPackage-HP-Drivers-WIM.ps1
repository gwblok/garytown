...
Notes to Self... I should parameterize parts of this, perhaps some don't want HPIA Offline Repo? Hmm....
#>
...
foreach ($Model in $HPModelsTable) #{Write-Host "$($Model.Name)"}
    ...
    $MaxOSSupported = ((Get-HPDeviceDetails -platform $Prodcode -oslist).OperatingSystem | Where-Object {$_ -notmatch "LTSB"}| Select-Object -Unique| measure -Maximum).Maximum
