...
$HPModelsSelectTable = Get-CMPackage -Fast -Name "HPIARepo*" | Where-Object {$_.Manufacturer -eq "HP" -and $_.Mifname -eq $Stage} |Select-Object -Property "Name","MIFFilename","PackageID", "Version","MifVersion" | Sort-Object $_.MIFFilename | Out-GridView -Title "Select the Models you want to Update" -PassThru
$HPModelsTable = Get-CMPackage -Fast -Name "HPIARepo*" | Where-Object {$_.PackageID -in $HPModelsSelectTable.PackageID}
...
foreach ($Model in $HPModelsTable) #{Write-Host "$($Model.Name)"}
    ...
    Get-HPDeviceDetails -Platform $Platform | Select-Object -Property "SystemID", "Name"  | Out-File -FilePath $ReadmeFilePath -Append
