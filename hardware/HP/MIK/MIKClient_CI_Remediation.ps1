...
#test for HPCMSL 
try {Get-HPDeviceDetails | Out-Null}
catch {Test-HPCMSL}
...
if (Test-Path -Path $HPIAInstallPath){
    ...
    if ($HPIAExtractedVersion -match $HPIAVersion){
        ...
        Write-Host "HPIA Installed $HPIAExtractedVersion isn't current, Upgrading to $HPIAVersion" -ForegroundColor Green
        Install-HPImageAssistant -Extract -DestinationPath $HPIAInstallPath
        }

    <# - Trigger HPIA
    $HpiaArgumentsList = "/Operation:Analyze /Action:List /Selection:All /Category:All /ReportFolder:C:\ProgramData\HP\MIK\HPIAReport /Silent /Wait"
    $HpiaApplication = "$HPIAInstallPath \HPImageAssistant.exe"
    Write-Host "Running $HpiaApplication $HpiaArgumentsList" -ForegroundColor Green
    Start-Process -FilePath $HpiaApplication -ArgumentList $HpiaArgumentsList -NoNewWindow -Wait
    #>

}
Else {
    Write-Host "HPIA is not on machine, MIK Install Probably Failed" -ForegroundColor Red
}
